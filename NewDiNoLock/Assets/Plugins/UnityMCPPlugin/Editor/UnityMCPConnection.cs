    ...
    public class UnityMCPConnection
    {
        ...
        private static async void ConnectToServer()
        {
            ...
            catch (OperationCanceledException)
            {
                ...
                isConnected = false;
                LogConnectionFailure(lastErrorMessage, null, isTimeout: true);
            }
            catch (WebSocketException we)
            {
                lastErrorMessage = $"[UnityMCP] WebSocket error: {we.Message}\nDetails: {we.InnerException?.Message}";
                isConnected = false;
                LogConnectionFailure(lastErrorMessage, we.StackTrace, isConnectionRefused: we.Message.Contains("refused", StringComparison.OrdinalIgnoreCase));
            }
            catch (Exception e)
            {
                lastErrorMessage = $"[UnityMCP] Failed to connect to MCP Server: {e.Message}\nType: {e.GetType().Name}";
                isConnected = false;
                LogConnectionFailure(lastErrorMessage, e.StackTrace, isConnectionRefused: false);
            }
        }

        private static void LogConnectionFailure(string message, string stackTrace, bool isConnectionRefused = false, bool isTimeout = false)
        {
            connectionFailureCount++;
            bool isFirstFailure = (connectionFailureCount == 1);
            bool shouldLogThisTime = isFirstFailure || (connectionFailureCount % LogRetryInterval == 0);
            ...
            {
                if (isFirstFailure)
                    Debug.LogWarning($"[UnityMCP] {message}\nMCP server is not running at {serverUri}. Start the MCP server (e.g. in Cursor) to connect. The plugin will keep retrying in the background.");
                else
                    Debug.LogWarning($"[UnityMCP] Still cannot connect to MCP server at {serverUri} (attempt #{connectionFailureCount}).");
            }
            else
            {
                Debug.LogError(message);
                if (isFirstFailure && !string.IsNullOrEmpty(stackTrace))
                    Debug.LogError($"[UnityMCP] Stack trace: {stackTrace}");
            }
        }
        ...
        private const int LogRetryInterval = 12; // Only log connection failure every 12 retries (~1 min)
